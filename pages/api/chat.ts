...
import { type Message, initialMessages, getResponse } from "../../components/chat-message";
...
export default async function handler(req: NextRequest) {
  ...
  const lastMessage = body.messages[body.messages.length - 1].message;

  // Check if the last message has a predefined response or is an unrelated question
  const predefinedResponse = getResponse(lastMessage);

  // If a predefined response is found, return it directly
  if (predefinedResponse) {
    return NextResponse.json({ text: predefinedResponse });
  }
